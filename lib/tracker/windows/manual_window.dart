            ...
            children: headers
                .map(
                  (h) => Padding(
                    ...
                    child: Text(
                      h,
                      style: trackerStyle(size: 15, color: kGreen),
                    ),
                  ),
                )
                .toList(),
              ...
              children: row
                  .map(
                    (cell) => Padding(
                      ...
                      child: Text(
                        cell,
                        style: trackerStyle(size: 15, color: Colors.white),
                      ),
                    ),
                  )
                  .toList(),
...
class _Block {
  const _Block.h1(this.text) : type = _BT.h1, rows = const [];
  const _Block.h2(this.text) : type = _BT.h2, rows = const [];
  const _Block.body(this.text) : type = _BT.body, rows = const [];
  const _Block.table(this.rows) : type = _BT.table, text = '';
...
const _song = [
  ...
  _Block.table([
    ...
    [
      'Replicate a chain',
      'REP — copies phrases to new empty phrase slots (M8-style)',
    ],
    ['Double-tap empty cell', 'Creates a new chain and advances cursor'],
  ]),
  _Block.h2('Song Settings'),
  _Block.body(
    'Open the ☰ menu → SONG SETTINGS to change global BPM (60-240) and LPB (lines per beat, 1-16).',
  ),
];
...
const _chain = [
  ...
  _Block.h2('Editing'),
  _Block.body(
    '+/- on PH to select a phrase. +/- on TR to transpose by semitones.',
  ),
  _Block.body('REP — duplicates the selected row. DEL — clears the row.'),
  _Block.h2('Chain FX'),
  _Block.body('BPM  LPB — see the FX sections.'),
];
...
const _phrase = [
  ...
  _Block.body(
    'OFF and END buttons appear in the edit bar when cursor is on NT.',
  ),
  _Block.body('Tap IN, VOL, or FX/VL cells → +/- to edit.'),
  _Block.body('Tap a row number to select it; tap again to extend selection.'),
  _Block.body(
    'DEL clears the column at the cursor row. REP duplicates the row.',
  ),
];
...
const _instrument = [
  ...
  _Block.body(
    'Tap the play button to audition the sample with current parameters.',
  ),
];
...
const _undoRedo = [
  ...
  _Block.body(
    'Note edits, FX edits, chain edits, song edits, mixer changes, instrument parameter changes.',
  ),
...
const _interpolation = [
  ...
  _Block.table([
    ...
    [
      '2',
      'Write the same FX command on a lower row with a different value (e.g. LPF 80)',
    ],
    ['3', 'Double-tap the lower value cell to open the edit menu'],
    [
      '4',
      'Tap INT — all rows between the two anchors are filled automatically',
    ],
  ]),
  _Block.body(
...
const _tips = [
  ...
  _Block.table([
    ...
    [
      '6',
      'Dial in the Mixer last — levels and reverb/delay once arrangement is locked',
    ],
